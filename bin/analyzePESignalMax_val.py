    ...
    for cluster_number in range(0,n_clusters):
        ...
        for i in range(0,len(labels)):
               if cluster_number == labels[i]:
                   ...
                   if d[i] > max_d_val:
                       max_d_val = d[i]
        QE = [max_d_val*1000*np.exp(drift_time/tau)/photons_per_pixel]
        QE_max_val += QE
        print(QE)
        
        n, bins = np.histogram(dvals, bins = 20)
        n_vals += [n]
        bin_vals += [bins]
    """
    #fig, axs = plt.subplots(2, 2)
    fig, axs = plt.subplots(1, 1)
    fig.suptitle("Charge Distributions")
    axs[0, 0].stairs(n_vals[0], bin_vals[0])
    axs[0, 0].set_title('Cluster 0')
    axs[0, 1].stairs(n_vals[1], bin_vals[1])
    axs[0, 1].set_title('Cluster 1')
    axs[1, 0].stairs(n_vals[2], bin_vals[2])
    axs[1, 0].set_title('Cluster 2')
    axs[1, 1].stairs(n_vals[3], bin_vals[3])
    axs[1, 1].set_title('Cluster 3')
    
    for ax in axs.flat:
        ax.set(xlabel='Charge (electrons)', ylabel='Counts')
...
if __name__ == "__main__":
    ...
    points = plt.scatter(xn, yn,c=color_array, s = 10)
    for i in range(0,len(x_centers)):
            plt.text(x_centers[i],y_centers[i],'Cluster '+str(i), fontsize = 'xx-small')
    plt.show()
    """
